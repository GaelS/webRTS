...
export default class FlowField {
    ...
    */
    updateDistanceValue( target ) {
        let { targetTileX, targetTileZ } = getCorrectedTiles( target, this.xMax, this.zMax, this.step );
    ...
    updateVectorField( target ) {
        ...
        let newGrid = this.grids[ idGrid ].map( ( row, i ) => {
            return row.map( ( cell, j ) => {
                ...
                cell.direction = getDirection(i, j, distance, this.xMax, this.zMax, this.step, this.grids[ idGrid ] ); 
                return cell;
            } );    
        } );
    ...
    getTile( position, target ) {
        let { targetTileX, targetTileZ } = getCorrectedTiles( target, this.xMax, this.zMax, this.step );
        const idGrid = `${ targetTileX }${ targetTileZ }`;
        return this.grids[ idGrid ][ getTileNumber( position.x, this.step ) ][ getTileNumber( position.z, this.step )];
    }
    cleanGrid( target ){
        let { targetTileX, targetTileZ } = getCorrectedTiles( target, this.xMax, this.zMax, this.step );    
    ...
    getTileCenter( position ){
        let { targetTileX, targetTileZ } = getCorrectedTiles( position, this.xMax, this.zMax, this.step );
...
function getCorrectedTiles( target, xMax, zMax, step ){
    ...
    let targetTileX = tmpTargetTileX === 0 ? 1 : tmpTargetTileX === xMax - 1 ? tmpTargetTileX -1 : tmpTargetTileX;
    let targetTileZ = tmpTargetTileZ === 0 ? 1 : tmpTargetTileZ === zMax - 1 ? tmpTargetTileZ -1 : tmpTargetTileZ;
    return {
        targetTileX,
        targetTileZ
    };    
