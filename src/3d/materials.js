...
function initMaterials(scene) {
  ...
  scene.materialsBlacker = new BABYLON.StandardMaterial(
    'blackerMaterial',
    scene
  );
  ...
  scene.selectionMaterial = new BABYLON.StandardMaterial(
    'selectionMaterial',
    scene
  );
  scene.selectionMaterial.diffuseColor = new BABYLON.Color3(0.2, 0.2, 0.2);
  scene.selectionMaterial.specularColor = new BABYLON.Color3(0, 0, 0);
  ...
  scene.materialsGreen.diffuseColor = new BABYLON.Color3(
    27 / 255,
    207 / 255,
    37 / 255
  );
  ...
  scene.materialsBlue.diffuseColor = new BABYLON.Color3(
    27 / 255,
    37 / 255,
    207 / 255
  );
...
function selectMeshes(scene, meshes) {
  return scene.meshes
    .filter(mesh => meshes.includes(mesh.name))
    .forEach(mesh => mesh.onSelect());
}
function deselectMeshes(scene, meshes) {
  return scene.meshes
    .filter(mesh => meshes.includes(mesh.name))
    .forEach(mesh => mesh.onDeselect());
}
export default {
  initMaterials,
  selectMeshes,
