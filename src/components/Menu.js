    ...
    <div style={S.menu}>
      ...
      {selectedElements.length !== 0 &&
        selectedElements.map((element, i) =>
          <div key={i}>
            {`${element.type.label} // ${element.life}${!!element.status
              ? ` // ${element.status}`
              : ''}`}
          </div>
        )}
...
const mapStateToProps = state => {
  let firstMesh =
    state.scene && state.selectedMeshes[0]
      ? state.scene.getMeshByID(state.selectedMeshes[0])
      : {};
  let IsAbuildingSelected =
    !!firstMesh.type && firstMesh.type.class === 'BUILDING';
  //GET CHARACTERS BEING CREATED IF ANY
  let characterBeingCreated = !IsAbuildingSelected
    ? []
    : _.map(firstMesh.characterOnCreation, type => type.label) || [];
